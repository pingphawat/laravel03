...
@section('content')
    ...
    <div>
    <a class="inline-block py-2 px-4 border border-gray-600 bg-red-600"
       href="{{ route('artists.edit', ['artist' => $artist]) }}">
        Edit Artist
    </a>
        @if($artist->songs->isEmpty())
        <form class="inline-black"
            action=" {{route('artists.destroy', ['artist' => $artist])}}" method="POST">
            @csrf
            @method("DELETE")
            <button  class="inline-block py-2 px-4 border border-gray-600 bg-green-600"
                type="submit"> Delete Artist
            </button>
        </form>
        @endif
    </div>
